...
class CartTest(BaseTest):
    def setUp(self):
        ...
        shop_creation = self.client.post(me_shop_list_create_url(), shop_create_payload())
        ...
        product_creation = self.client.post(product_create_url(self.shop.uuid), product_payload)
...
# ORDER TESTS
    ...
    def test_order_item_review(self):
        order = self.test_order_create()
        order_item = OrderItem.objects.filter(order= order).first()
        response = self.client.patch(urlhelpers.order_item_review_url(order_item.uuid), payloads.order_item_review_payload())
        print(json.dumps(response.data, indent=4))