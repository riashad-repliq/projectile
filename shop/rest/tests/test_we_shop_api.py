...
class WeShopTest(BaseTest):
    def setUp(self):
        ...
        shop_creation_1 = self.client.post(urlhelpers.me_shop_list_create_url(), payloads.shop_create_payload())
        self.shop_1 =get_object_or_404(Shop, uuid=shop_creation_1.data['uuid'])

        shop_creation_2 = self.client.post(urlhelpers.me_shop_list_create_url(), payloads.shop_create_payload())
        self.shop_2 =get_object_or_404(Shop, uuid=shop_creation_2.data['uuid'])

        shop_creation_3 = self.client.post(urlhelpers.me_shop_list_create_url(), payloads.shop_create_payload())
